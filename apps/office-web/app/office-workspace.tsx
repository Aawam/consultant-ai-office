...
function ExportPanel({ role, status, onAction }: { role: Role; status: Status; onAction: (message: string) => void }) { return <section className="panel export-panel"><PanelHeading eyebrow="EXPORT WORKSPACE" title="Separate output contracts" chip="NOT IMPLEMENTED" /><div className="export-grid"><div><span className="severity-tag info">WORKING EXPORT · NOT OFFICIAL</span><p>Allowed for DRAFT/REVIEW and TECHNICAL/ADMIN.</p><button type="button" className="secondary-button" disabled={status === "FINAL"} onClick={() => onAction("Working Excel exporter belum tersedia; tidak ada file yang dibuat.")}>Export working Excel</button></div><div><span className="severity-tag warning">OFFICIAL EXPORT</span><p>ADMIN + FINAL only. Production exporter belum tersedia.</p><button type="button" className="secondary-button" disabled={role !== "ADMIN" || status !== "FINAL"} onClick={() => onAction("Official Excel exporter belum tersedia; tidak ada success palsu.")}>Export official Excel</button></div></div></section>; }
